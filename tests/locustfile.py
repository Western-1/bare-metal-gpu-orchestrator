...
class EmbeddingUser(HttpUser):
    ...
    def embed_short_text(self):
        ...
        with self.client.post(
            "/embed", json=payload, catch_response=True, name="/embed (short)"
        ) as response:
    ...
    def embed_long_text(self):
        ...
        with self.client.post(
            "/embed", json=payload, catch_response=True, name="/embed (long)"
        ) as response:
            if response.status_code != 200:
                response.failure(f"HTTP {response.status_code}")
...
def on_test_stop(environment, **kwargs):
    ...
    if environment.stats.total.fail_ratio > 0.01:
        print(
            f"WARNING: Failure rate {environment.stats.total.fail_ratio:.2%} exceeds 1%"
        )
    print(f"Total Requests: {environment.stats.total.num_requests}")
    print(f"Total Failures: {environment.stats.total.num_failures}")
    print(f"RPS: {environment.stats.total.total_rps:.2f}")
    print(
        f"P95 Latency: {environment.stats.total.get_response_time_percentile(0.95):.0f}ms"
    )
    print("=" * 50 + "\n")
